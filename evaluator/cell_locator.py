...
def build_activity_mapping(file_path, sheet_name="EVALUACIÓN",
    ...
    # Recorrer todos los rangos combinados de la hoja
    for merged_range in ws.merged_cells.ranges:
        ...
        if min_row == header_rows[0] and max_row == header_rows[1]:
            ...
            if header_value and merged_header_text.lower() in str(header_value).lower():
                # Se ha identificado un bloque de actividades: recorrer cada columna de este bloque
                for col in range(min_col, max_col + 1):
                    ...
                    if cell.value is not None:
                        ...
                        # Si ya existe la actividad, se agrega la referencia a la lista;
                        # de lo contrario, se crea una nueva entrada en el diccionario.
...
def map_activities():
    """Construye el mapeo de actividades y lo muestra en una ventana emergente."""
    ...
    if mapping:
        ...
        for activity, cell_refs in mapping.items():
            result_text += f"  {activity}: {cell_refs}\n"
    else:
